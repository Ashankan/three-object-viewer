			...
			<InspectorControls>
				<Panel header={__("Road Settings", "three-object-viewer")}>
					<PanelBody title={__("Geometry", "three-object-viewer")} initialOpen={true}>
						<PanelRow>
							<RangeControl
								label={__("Road Width", "three-object-viewer")}
						...
						<PanelRow>
							<RangeControl
								label={__("Segments", "three-object-viewer")}
						...
						<PanelRow>
							<RangeControl
								label={__("Units Per Second", "three-object-viewer")}
						...
						<PanelRow>
							<TextControl
								label={__("Duration (seconds)", "three-object-viewer")}
								type="number"
					...
					<PanelBody title={__("Waveform Texture", "three-object-viewer")} initialOpen={false}>
						<PanelRow>
						...
						{attributes.waveformUrl && (
							<PanelRow>
								<Button
									isDestructive
									isSmall
									onClick={() => setAttributes({ waveformUrl: "" })}
								>
					...
					<PanelBody title={__("Camera", "three-object-viewer")} initialOpen={false}>
						...
						<PanelRow>
							<RangeControl
								label={__("Look Ahead (segments)", "three-object-viewer")}
						...
						<PanelRow>
							<RangeControl
								label={__("FOV", "three-object-viewer")}
									...
									<div key={i} style={{ display: "grid", gridTemplateColumns: "44px 1fr 1fr 1fr 24px", gap: "4px", alignItems: "center", borderBottom: "1px solid #2a2a2a", padding: "3px 0" }}>
										...
										{pts.length > 2 ? (
											<button
												...
												style={{ border: "none", background: "transparent", color: "#888", cursor: "pointer", fontSize: "14px" }}
											>×</button>
										) : <span />}
									</div>
								))}
